...
- **Done**: 39 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #40a visual asset schema, #40b image backend, #40c asset persistence, #40d apply integration, #41a visual reviewer spec, #41b browser-audit skill, #41c proposal-intent verifier, #41d visual-review integration, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 17 (1 submission form + 5 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 42  | in-progress | **Analytics ingestion** — CF Worker pixel → D1 (or PostHog/GA4 webhook). Normalizes events to `{version_sha, metric, value, timestamp}`. Metric tiers: proxies (fast) vs CVR (slow confirmation).                                                                                                                                                                                                                                                                                             | 3     |
