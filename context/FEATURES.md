...
- **Done**: 34 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #40a visual asset schema, #40b image backend, #40c asset persistence, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 22 (1 submission form + 2 Layer 8 + 9 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 40d | in-progress | **#39 integration pattern** — apply worker's stub-when-absent / invoke-when-present flow (Q3 soft dependency). Apply emits `<!-- asset TBD: <type> -->` comments when #40 is unavailable, swaps them for real asset URLs after #40 ships.                                                                                                                                                                                                                                                                                                                                                 | 1     |
