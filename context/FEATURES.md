...
| 39  | todo   | **Apply step + review/fix loop — NEEDS PLANNING.** Worker reads latest `history/<week>/proposal.md` + `decision.json`, mutates `site/` per the selected issues, runs a Forge-style review→fix→re-review loop on the resulting diff, opens a second "apply" PR with real code changes (separate from the brief PR session 3 merged). Open questions: (a) which worker runtime (Pi/Codex/`claude -p` subscription?), (b) how the review step composes with existing critic findings vs. a new code-focused reviewer, (c) iteration cap + drift protection, (d) what counts as "done" (lint+type+visual-regression vs. critic sign-off), (e) preview URL strategy for the operator to review the apply PR before merge. Will consume #40 as a soft dependency. | TBD   |
| 40  | todo   | **Image-gen tool for #39 — NEEDS PLANNING.** Single tool `generate_visual_asset(type, brand_context, dims, prompt)` callable by the apply worker where `type ∈ {og_card, icon, hero_bg, section_illustration}` (enumerated to prevent scope creep). Backend candidate: OpenAI `gpt-image-1`. Open questions: (a) exact enumerated type list, (b) backend (OpenAI vs. Stitch MCP vs. Replicate), (c) brand-context input format (reference images? colour tokens? design-system JSON?), (d) per-week cost ceiling, (e) asset persistence path (`site/public/assets/generated/<week>/…`). Week 1 proposal already surfaced one concrete need (og-card.jpg). Soft dependency of #39 — #39 ships first with stubs, adopts #40 when it exists.                   | TBD   |
