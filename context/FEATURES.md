...
| 44  | done   | **Verdict engine (reward+gates 7-outcome matrix)** — `scripts/verdict-engine.ts` compares analytics windows to per-experiment baselines and emits `{verdict, confidence, reward_delta, gate_status[], lane}` where `lane ∈ {fast-track-promote, fallback-promote, gate-win, archive-gate-fail, auto-rollback, hold, hold-weak-negative}`. Current `confidence` is a heuristic operator-priority score, not statistical significance; production stats policy is deferred until real analytics volume exists. Includes cross-experiment page-CTR gate. | 4     |
