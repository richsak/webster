...
- **Done**: 29 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 27 (1 submission form + 7 Layer 8 + 9 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 39d | in-progress | **Per-cluster PR emission** — issue-cluster heuristic (union-find on touched files, 1–3 issues per PR, ≤3 PRs/week) per Q4. Severity-tiered skip+annotate fallback (Q5); `[partial]` label when any issue skipped; draft PR when a CRITICAL is skipped.                                                                                                                                                                                                                                                                                                                                   | 3     |
