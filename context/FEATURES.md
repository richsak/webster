...
- **Done**: 38 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #40a visual asset schema, #40b image backend, #40c asset persistence, #40d apply integration, #41a visual reviewer spec, #41b browser-audit skill, #41c proposal-intent verifier, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 18 (1 submission form + 1 Layer 8 + 6 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 41d | in-progress | **#39 integration pattern** — visual-reviewer runs AFTER #39c critic re-run gate, BEFORE #39d PR emission. Fix-hint loop back to #39a apply worker, max 3 iterations; then skip+annotate per #39d severity-tiered fallback. Blocking on CRITICAL visual regressions.                                                                                                                                                                                                                          | 1     |
