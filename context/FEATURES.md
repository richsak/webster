...
- **Done**: 37 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #40a visual asset schema, #40b image backend, #40c asset persistence, #40d apply integration, #41a visual reviewer spec, #41b browser-audit skill, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 19 (1 submission form + 1 Layer 8 + 7 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 41c | in-progress | **Proposal-intent verifier** — reads each issue in `proposal.md`, verifies visible presence in rendered output via accessibility-tree extraction (not source grep). Catches content drops like session-4 "No more patient churn" regression. Flags layout overflow via per-breakpoint height-delta vs BEFORE.                                                                                                                                                                                 | 2     |
