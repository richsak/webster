...
- **Done**: 31 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 25 (1 submission form + 5 Layer 8 + 9 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 40a | in-progress | **Tool schema + enumerated type list** — `generate_visual_asset(type, brand_context, dims, prompt)` with `type ∈ {og_card, hero_background, testimonial_headshot, icon, section_illustration}` (5-type enum from Q3). Emits stub-comment if unknown type.                                                                                                                                                                                                                                                                                                                                 | 1     |
