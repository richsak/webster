...
- **Done**: 32 (incl. #5 live run artifacts, #38 site/ fork shipped session 4, #39b runtime gate, #39c critic rerun gate, #39d PR emission plan, #39e CF preview wiring, #40a visual asset schema, #50 planner agent spec, and #51 memory substrate shipped)
- **In-progress**: 0
- **Blocked**: 5 (demo video — Richie voice)
- **Cut**: 7 (out of submission scope; rationale inline)
- **Todo**: 24 (1 submission form + 4 Layer 8 + 9 Layer 9 + 3 Layer 10 + 7 Layer 11; all pre-submission per session-4 Phase 7 scope update + Q1–Q9 locks)
...
| 40b | in-progress | **Backend wire-up** — OpenAI `gpt-image-1` client (default per Q3; `[R-confirm]`) + retry + cost ceiling of $2/run. Structured error handling for rate limits + NSFW filters. Falls back to stub-comment on hard failure.                                                                                                                                                                                                                                                                                                                                                                 | 3     |
